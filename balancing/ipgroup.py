...
def fraud_ratio(ip, transaction):
	if len(ip)==0:
		return 0
	return sum(map(lambda t: 1 if t[9]=="Chargeback" else 0, ip))/len(ip)
def average_amount(ip, transaction):
	if len(ip)==0:
		return 0
	return sum(map(lambda t: t[5], ip))/len(ip)
	return 0
def average_num_of_daily_transactions(ip, transaction):
	if len(ip)==0:
		return 0
...
for i, transaction in enumerate(data[data.columns.values].values):
	...
	data.iat[i,19] = fraud_ratio(ip, transaction)
	data.iat[i,20] = average_amount(ip, transaction)
	data.iat[i,21] = average_num_of_daily_transactions(ip, transaction)
